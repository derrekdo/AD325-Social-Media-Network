...
public class DirectedGraph<T> implements GraphInterface<T>
{
    ...
    public boolean removeVertex(T vertexLabel){
        VertexInterface<T> removedVertex = vertices.remove(vertexLabel);
    ...
    public QueueInterface<T> getBreadthFirstTraversal(T origin)
    {
        ...
        resetVertices();
        //Queue to hold trvaersal order
        QueueInterface<T> traversalOrder = new LinkedQueue<>();
        //Queue to hold vertices as theyre visited
        QueueInterface<VertexInterface<T>> vertexQueue = new LinkedQueue<>();

        //hold the vertex and marks it as visited
        VertexInterface<T> originVertex = vertices.getValue(origin);
        originVertex.visit();
        //queue to hold traversal order and visited vertex
        traversalOrder.enqueue(origin);
        vertexQueue.enqueue(originVertex);

        //checks if the queue of vertices is empty
        while(!vertexQueue.isEmpty()){
            //removes the front vertex from queue
            VertexInterface<T> frontVertex = vertexQueue.dequeue();
            //checks if that vertex has a neighbor
            while(frontVertex.hasNeighbor()){
                //neighbor of front vertex
                VertexInterface<T> nextNeighbor = frontVertex.getUnvisitedNeighbor();

                //chekcs if neighbor is null to prevent infinite loop
                ...
                if(!nextNeighbor.isVisited()){
                    //mark as visited
                    nextNeighbor.visit();
                    //add the vertex to traversal que
                    traversalOrder.enqueue(nextNeighbor.getLabel());
                    //add vertex to visited queue
